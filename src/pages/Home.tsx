...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (c.length === 8) { setLoading(true); fetchCep(c).then(a => { setAddr(a); toast[a?"success":"error"](a?"Endereço encontrado!":"CEP não encontrado"); }).finally(() => setLoading(false)); }
    else setAddr(null);
  }, [cep]);
          ...
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            {imgs.length > 0 && (
              <React.Fragment>
                ...
                <button onClick={process} disabled={proc} className="w-full bg-yellow-400 text-gray-900 font-bold py-4 rounded-xl hover:bg-yellow-300 disabled:opacity-50 flex items-center justify-center gap-2">{proc ? <React.Fragment><Loader2 className="w-5 h-5 animate-spin" />Processando {prog.c}/{prog.t}...</React.Fragment> : <React.Fragment><Route className="w-5 h-5" />Calcular Rota ({imgs.length})</React.Fragment>}</button>
              </React.Fragment>
            )}
          </div>
        )}
                ...
                <div key={d.id} className={`rounded-xl border-2 p-4 ${d.status === "entregue" ? "bg-green-500/10 border-green-500" : d.status === "nao-entregue" ? "bg-red-500/10 border-red-500" : "bg-blue-500/10 border-blue-500"}`}>
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-O arquivo foi cortado. Vou completá-lo:

<dyad-write path="src/pages/Home.tsx" description="Complete Home.tsx file">
import React, { useState, useEffect, useRef } from "react";
import { toast, Toaster } from "sonner";
import { MapPin, Loader2, RotateCcw, Info, Zap, Camera, Upload, Navigation, Trash2, CheckCircle2, XCircle, Clock, Route, Images, X } from "lucide-react";
        ...
        {addr && dels.length === 0 && (
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            {imgs.length > 0 && (
              <div>
                <div className="grid grid-cols-4 gap-2 mb-4">{imgs.map(i => <div key={i.id} className="relative"><img src={i.base64} className="w-full h-16 object-cover rounded-lg" /><button onClick={() => setImgs(p => p.filter(x => x.id !== i.id))} className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center"><X className="w-3 h-3 text-white" /></button></div>)}</div>
                <button onClick={process} disabled={proc} className="w-full bg-yellow-400 text-gray-900 font-bold py-4 rounded-xl hover:bg-yellow-300 disabled:opacity-50 flex items-center justify-center gap-2">{proc ? <span className="flex items-center gap-2"><Loader2 className="w-5 h-5 animate-spin" />Processando {prog.c}/{prog.t}...</span> : <span className="flex items-center gap-2"><Route className="w-5 h-5" />Calcular Rota ({imgs.length})</span>}</button>
              </div>
            )}
          </div>
        )}
