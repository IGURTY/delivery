...
const Home: React.FC = () => {
  ...
  const loadActiveRoute = async () => {
    ...
    try {
      ...
      if (activeRoute) {
        ...
        setRouteGenerated(activeRoute.total_deliveries > 0);
  ...
  const handleCapture = async (imageBase64: string) => {
    ...
    try {
      ...
      if (routeId) {
        ...
        if (dbDelivery) newDelivery.id = dbDelivery.id;
      }
  ...
  const handleAttachProof = async (id: string, file: File) => {
    ...
    reader.onload = async (ev) => {
      if (ev.target?.result) {
        ...
        setDeliveries((prev) =>
          prev.map((d) => (d.id === id ? { ...d, status: "entregue" as DeliveryStatus, proofImage } : d))
        );
  ...
  const handleReset = async () => {
    if (currentRouteId) await db.updateRouteStatus(currentRouteId, "cancelled");
    setDeliveries([]);
    setStartCep("");
    ...
    <div className="min-h-screen bg-dark">
      ...
      <header className="bg-gray-900 border-b border-gray-800 sticky top-0 z-50">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={loadRouteHistory} className="p-2 text-gray-400 hover:text-primary">
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 rounded-xl max-w-lg w-full max-h-[80vh] overflow-auto">
            <div className="p-4 border-b border-gray-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowHistory(false)} className="text-gray-400 hover:text-white text-xl">
                ×
              </button>
            </div>
        ...
        {deliveries.length > 0 && (
          <div className="grid grid-cols-4 gap-2">
            <div className="bg-gray-900 rounded-lg p-3 text-center border border-gray-800">
              <p className="text-Vou finalizar o arquivo Home.tsx:

<dyad-write path="src/pages/Home.tsx" description="Home.tsx completo e funcionando">
import React, { useState, useEffect } from "react";
import { toast, Toaster } from "sonner";
import ImageCapture from "@/components/ImageCapture";
import DeliveryCard, { Delivery, DeliveryStatus } from "@/components/DeliveryCard";
              ...
              {routeHistory.length === 0 ? (
                ...
                routeHistory.map((route) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-500 mt-2">{new Date(route.created_at).toLocaleDateString("pt-BR")}</p>
                  </button>
                ))
              )}
            </div>
